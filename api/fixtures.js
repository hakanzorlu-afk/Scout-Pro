module.exports = async (req, res) => {
  ...
  const KEY = process.env.ALLSPORTS_KEY || process.env.BZZOIRO_KEY;
  if (!KEY) return res.status(500).json({ error: 'API KEY eksik - ALLSPORTS_KEY veya BZZOIRO_KEY gerekli' });
  ...
  try {
    ...
    if (!data.success) throw new Error(data.message || 'API hatasi: ' + JSON.stringify(data));
